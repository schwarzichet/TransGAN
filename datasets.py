...
from celeba import CelebA, FFHQ, Table, AdultTable
    ...
    def __init__(self, args, cur_img_size=None, bs=None):
        ...
        if args.dataset.lower() == 'cifar10':
            ...
            self.test = torch.utils.data.DataLoader(
                ...
                batch_size=args.dis_batch_size, shuffle=False,
                num_workers=args.num_workers, pin_memory=True, sampler=val_sampler)
        elif args.dataset.lower() == 'asia_table':
            Dt = Table
            
            train_dataset = Dt(root=args.data_path)
            val_dataset = Dt(root=args.data_path)
            ...
            self.test = torch.utils.data.DataLoader(
                ...
                batch_size=args.dis_batch_size, shuffle=False,
                num_workers=args.num_workers, pin_memory=True, sampler=val_sampler)
        # elif args.dataset.lower() == 'asia_a':
        #     Dt = CelebA
        #     transform = transforms.Compose([
        ...
        #         num_workers=args.num_workers, pin_memory=True, sampler=val_sampler)
            ...
            self.cat_size,self.cats, self.num_size = train_dataset.get_cat_num_size()

            self.get_cat_num_size = train_dataset.get_cat_num_size
            self.old_header = train_dataset.old_header
            self.new_header = train_dataset.new_header
