...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("length of data_loader: ", len(train_loader))
    if args.max_iter:
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            gen_net = torch.nn.parallel.DistributedDataParallel(
                gen_net, device_ids=[args.gpu], find_unused_parameters=True
            )
            dis_net = torch.nn.parallel.DistributedDataParallel(
                dis_net, device_ids=[args.gpu], find_unused_parameters=True
            )
        else:
            gen_net.cuda()
            dis_net.cuda()
